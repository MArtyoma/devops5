...
def test_create_user_with_valid_email():
    ...
    new_user = {'name': 'Alexei Alekseev', 'email': 'a.a.alekseev@mail.com'}
    response = client.post("/api/v1/user", json=new_user)
    assert response.status_code == 201
    # Проверяем, что вернулся id нового пользователя (целое число)
...
def test_create_user_with_invalid_email():
    ...
    existing_user = {'name': 'Duplicate', 'email': users[1]['email']}
...
def test_delete_user():
    ...
    user_to_delete = {'name': 'Delete Me', 'email': 'delete.me@mail.com'}
